...
def registration_collate_fn_stack_mode(
    ...
    feats = torch.cat(collated_dict.pop('tgt_feats') + collated_dict.pop('src_feats'), dim=0)
    pcd_list = collated_dict.pop('tgt_pcd') + collated_dict.pop('src_pcd')
    lengths = torch.LongTensor([points.shape[0] for points in pcd_list])
    cat_pcd = torch.cat(pcd_list, dim=0)
    ...
    if precompute_data:
        input_dict = precompute_data_stack_mode(cat_pcd, lengths, num_stages, voxel_size, search_radius, neighbor_limits)
        collated_dict.update(input_dict)
    else:
        collated_dict['pcd'] = cat_pcd
...
def grid_subsample(points, lengths, voxel_size):
    ...
    """
    ext_module = importlib.import_module('extensions.ext')
