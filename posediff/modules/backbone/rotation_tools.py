...
# no batch        
def compute_ortho6d_from_rotation_matrix_tensor(r_matrix):
    return torch.cat((r_matrix[:, 0], r_matrix[:, 1]), axis=0)

def compute_ortho6d_from_rotation_matrix_np(r_matrix):
    return np.concatenate((r_matrix[:, 0], r_matrix[:, 1]), axis=0)
...
if __name__ == '__main__':
    rotation_matrix = np.array([[0.0, 0.7, 0.1],
                                [0.03, 0.9, 0.0],
                                [-0.4, 0.0, 0.3]])
    rotation_matrix = torch.from_numpy(rotation_matrix).float().cuda()
    ortho6d = compute_ortho6d_from_rotation_matrix_tensor(rotation_matrix)
    ortho6d = ortho6d.unsqueeze(0)
    rebuild_matrix = compute_rotation_matrix_from_ortho6d(ortho6d)
    print(rebuild_matrix)
